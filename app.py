...
# ETF data
etfs = {
    "Global X Robotics & AI (BOTZ)": {"ticker": "BOTZ", "expense_ratio": "0.69%", "focus": "Robotics + AI hardware"},
    "ARK Autonomous Tech & Robotics (ARKQ)": {"ticker": "ARKQ", "expense_ratio": "0.75%", "focus": "Disruptive AI + Automation"},
    "iShares AI ETF (IRBO)": {"ticker": "IRBO", "expense_ratio": "0.47%", "focus": "Broad AI + Small Caps"},
}
...
amount = st.number_input("Investment Amount ($ per month for SIP / one-time for Lumpsum)", min_value=50, value=500)
...
if data.empty:
    ...
    if investment_mode == "SIP":
        ...
        df_plot = pd.DataFrame({"Month": data.index.strftime("%b %Y"),
    ...
    # Plotly chart
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_plot["Month"], y=df_plot["Invested Amount"], mode='lines+markers', name='Invested Amount'))
    fig.add_trace(go.Scatter(x=df_plot["Month"], y=df_plot["Market Value"], mode='lines+markers', name='Market Value'))

    fig.update_layout(title=f"{selected_etf} - {investment_mode} Growth",
                      xaxis_title="Month",
                      yaxis_title="Amount ($)",
                      hovermode="x unified")
    ...
    st.write(f"**Total Invested:** ${invested[-1]:,.2f}")
    st.write(f"**Current Value:** ${values[-1]:,.2f}")
    st.write(f"**Return:** {((values[-1] - invested[-1]) / invested[-1]) * 100:.2f}%")
